function [ ] = fn_CPM_plot_bestfit(do_plot, save_T_stat, C_cookd, C_ZRE,  Tout, y_mea, pathnm_csv, pathnm_pics, Case_str, date_start, date_end, CPM_type_best, date_index)
...
%% y_mea nan 값은 0으로 해서 보여줌
[~, idx_nan] = fn_nan2zero(y_mea);
fnExistFolder(pathnm_pics);

%% CPM 선택
% (210302) 4parameter 모델은 고려하지 않음
% CPM_type_best={'1p','2p_h','2p_c','3p_h','3p_c','4p_h','4p_c','5p'};
% CPM_type_best={'2p_h','2p_c','3p_h','3p_c','5p'};
...
switch do_plot 
    ...
    case 1
        ...
        for m=1:length(CPM_type_best)
            CPM_type=CPM_type_best{m};
                ...
                %% 산점도 표기 
%                 if strcmp(flag, 'd')
%                     scatter(Tout, y_mea,'MarkerFaceColor',[0 0.4470 0.7410], 'MarkerEdgeAlpha', 0.5) 
                    gscatter(Tout', y_mea', date_index');
...
%                 end
                ...
                hold on
                scatter(Tout(idx_nan),y_mea(idx_nan), 'MarkerFaceColor', 'r') 
...
%                         scatter(Tout,y_mea,'k')
                ...
                %  gscatter 레전드 설정
%                 if numel(Tout) < 37
                    a.Legend.String =  string (unique(date_index));

%                 elseif unique(date_index) = 7
%                     a.Legend.String =  string (unique(date_index));
%                 elseif unique(date_index) = 2
